...
extern "C" widget_t rmp_load_widget(char* widget_id, WIDGET_TYPE widget_type ) {
...
WIDGET_TYPE rmp_widget_stype2type(char *type) {
    if (type == NULL || strlen(type)==0) return UnknownWidgetType;

    if (strcmp(type, "HorizontalLayout") == 0) return HorizontalLayout;
    if (strcmp(type, "VerticalLayout") == 0) return VerticalLayout;
    if (strcmp(type, "ROSVideo") == 0) return ROSVideo;
    if (strcmp(type, "ROSAttitude") == 0) return ROSAttitude;
    if (strcmp(type, "ROSHeading") == 0) return ROSHeading;
