...
static esp_err_t cam_dma_config(const camera_config_t *config)
{
    ...
    printf("cam dma buffer_size: %d, half_buffer_size: %d, node_buffer_size: %d, node_cnt: %d, total_cnt: %d\r\n",
    ...
    if (cam_obj->frames == NULL) {
        printf("Frames malloc fail\r\n");
        return ESP_FAIL;
    }
    ...
    //    CAM_CHECK(cam_obj->frames[x].fb.buf != NULL, "frame buffer malloc failed", ESP_FAIL);
        ...
        if (cam_obj->psram_mode) {
            ...
            printf("Frame[%d]: Offset: %u, Addr: 0x%08X\r\n", x, cam_obj->frames[x].fb_offset, (unsigned) cam_obj->frames[x].fb.buf);
            ...
            if(cam_obj->frames[x].dma == NULL) {
                printf("Frame DMA Malloc failed\r\n");
                return ESP_FAIL;
            }
        }
...
esp_err_t cam_init(const camera_config_t *config)
{
    ...
    if (NULL == cam_obj) {
        printf("Error alloc ram for camera\r\n");
        goto err;
    }
    cam_obj->swap_data = 0;
...
//    CAM_CHECK_GOTO(cam_obj->event_queue != NULL, "event_queue create failed", err);
    if (cam_obj->event_queue == NULL) {
        printf("event queue fail\r\n");
        goto err;
    }
    ...
    if (cam_obj->frame_buffer_queue == NULL) {
        printf("frame buffer fail\r\n");
        goto err;
    }
