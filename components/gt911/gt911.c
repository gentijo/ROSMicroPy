...
esp_err_t gt911_init(i2c_bus_handle_t i2c_bus, const gt911_config_t *config) {
  ...
  if (GPIO_IS_VALID_GPIO(config->int_pin)) {    
    printf("Configuring GPIO to be floating input\r\n");
    ...
    if (result != ESP_OK) printf("Error during GPIO config\r\n");
  ...
  if (err == ESP_OK) {
    ...
    err = i2c_bus_read_bytes_16(_i2c_dev, GT911_PRODUCT_ID_REG, 4, &buf[0]);
    if (err == ESP_OK) {
      printf("GT911 device found at addr: 0x14\r\n");
    } else {
      printf("GT911 device not found!\r\n");
      return err;
    }
  }
  
  // Display Product ID
  printf("GT911 Product ID: 0x%02x,0x%02x,0x%02x,0x%02x\r\n", buf[0], buf[1], buf[2], buf[3]);

  // Display Config
  err = i2c_bus_read_bytes_16(_i2c_dev, GT911_CONFIG_REG, 1, &buf[0]);
  if (err != ESP_OK) return err;
  printf("GT911 Config: 0x%02x\r\n", buf[0]);
